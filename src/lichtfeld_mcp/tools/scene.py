...
def open_project(path: str) -> dict:
    ...
    return get_scene_api().open_project(path).model_dump()


def save_project() -> dict:
    """Save the currently open project."""

    return get_scene_api().save_project().model_dump()


def close_project() -> dict:
    """Close the currently open project."""

    return get_scene_api().close_project().model_dump()


def get_scene_stats() -> dict:
    """Return scene statistics: splat count, selection count, bounds, VRAM estimate."""

    return get_scene_api().get_scene_stats().model_dump()


def list_history() -> list[dict]:
    """Return the editor operation history."""

    return [entry.model_dump() for entry in get_scene_api().list_history()]


def undo() -> dict:
    """Undo the last scene operation when possible."""

    return get_scene_api().undo().model_dump()
